...
export const load: PageServerLoad = async ({ locals: { supabase } }) => {
	const { data, error } = await supabase
		.from('case_projects')
		.select('id, slug, title, category, year, sort_order')
...
export const actions: Actions = {
	delete: async ({ request, locals: { supabase } }) => {
		...
		const { error } = await supabase.from('case_projects').delete().eq('id', id);
	...
	reorder: async ({ request, locals: { supabase } }) => {
		...
		const results = await Promise.all(
			ids.map((id, index) =>
				supabase.from('case_projects').update({ sort_order: index }).eq('id', id)
			)
		);
